...
| `T-09` | A short/long control over the EVENT and HEAT headers only, starting from short | should | `diverges` | picker menu → `AppModel.setLabelStyle`, options `prefs_short` / `prefs_long`. **Starts from long, not short, and the third row is gone**: app.md offers `prefs_auto` ("Meet default"), which followed the operator's `settings.label_style`. Product decision 2026-09-13 — the device's style always wins and long is the default. `settings.label_style` is still decoded, just no longer consulted. app.md T-09 needs the same edit |
