...
| `P-15` | The app's own light/dark, chosen in the picker's menu: Dark, Light, Automatic | native-only | `done` | `Appearance` in `Preferences`, set through `AppModel.setAppearance`, resolved to a `ColorScheme?` in **one place** — `SplouchRootView.scheme`. Inside a meet the board decides (`RGBA(hex: ctx.colors.bg).isDark`): a meet themes itself (§7), so the chrome over it follows the board rather than a device preference that would fight it. Outside a meet it is the user's choice, `auto` resolving to nil so the device decides. **Dark is the default**, including for preferences stored before the key existed — the picker was pinned dark since the web page it came from and a spectator who never opens this menu should see the app they saw yesterday. The pin is what made this worth doing: `.preferredColorScheme(.dark)` sat on the stack the shell is pushed into and beat the shell's own choice, so a meet themed light (the server ships `white.toml`) drew light rows inside a black navigation bar, a black tab bar and a black background, under its own dark header text. The words are the app's (T-05) — light and dark are about the device, and no server serves them. Tests: `storedPreferencesWithoutAnAppearanceStayDark`, `languageAndLabelStylePersist` |
